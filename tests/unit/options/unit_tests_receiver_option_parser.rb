...
class UnitTestsReceiverOptionParser < Minitest::Test
  ...
  def test_receiver_option_parser_user_timeout_value_short_float
    user_receiver_options_timeout_short_float = \
      Options::ReceiverOptionParser.new(["-t", "123.4"])
    assert_equal(
      123.4,
  ...
  def test_receiver_option_parser_user_timeout_value_long_float
    user_receiver_options_timeout_long_float = \
      Options::ReceiverOptionParser.new(["--timeout", "123.1"])
    assert_equal(
      123.1,
  ...
  def test_receiver_option_parser_default_sasl_enabled_value
    default_receiver_options_sasl_enabled = Options::ReceiverOptionParser.new([])
    assert_equal(
      Defaults::DEFAULT_SASL_ENABLED,
      default_receiver_options_sasl_enabled.options.sasl_enabled
    )
  end # test_receiver_option_parser_default_sasl_enabled_value

  def test_receiver_option_parser_user_sasl_enabled_no_value
    receiver_options_user_sasl_enabled_no_value = Options::ReceiverOptionParser.new(
        ["--conn-sasl-enabled"]
    )
    assert_equal(
        true,
        receiver_options_user_sasl_enabled_no_value.options.sasl_enabled
    )
  end # test_receiver_option_parser_user_sasl_enabled_no_value

  def test_receiver_option_parser_user_sasl_enabled_value_long
    Options::BOOLEAN_STRINGS.each do |boolean_value|
      user_receiver_options_sasl_enabled_long = Options::ReceiverOptionParser.new(
        ["--conn-sasl-enabled", "#{boolean_value}"]
      )
      assert_equal(
        StringUtils.str_to_bool(boolean_value),
        user_receiver_options_sasl_enabled_long.options.sasl_enabled
      )
    end
  end # test_receiver_option_parser_user_sasl_enabled_value_long

  def test_receiver_option_parser_user_sasl_enabled_value_long_raise
    assert_raises OptionParser::InvalidArgument do
      Options::ReceiverOptionParser.new(
        ["--conn-sasl-enabled", "raise"]
      )
    end
  end # test_receiver_option_parser_user_sasl_enabled_value_long_raise

  def test_receiver_option_parser_user_sasl_enabled_value_long_raise_message
    wrong_value = "raise"
    exception = assert_raises OptionParser::InvalidArgument do
      Options::ReceiverOptionParser.new(
  ...
  end # test_receiver_option_parser_user_sasl_enabled_value_long_raise_message
